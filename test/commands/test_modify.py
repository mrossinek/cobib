...
class TestModifyCommand(CommandTest, TUITest):
    ...
    def test_command(self, setup, modification, filters, selection):
        ...
        if git:
            ...
            self.assert_git_commit_message('modify', {'append': False,
                                                      'selection': selection,
    ...
    def test_append_mode(self, setup, caplog):
        ...
        assert ('cobib.commands.modify', 30, 'The append-mode of the `modify` command has not been'
                ' implemented yet.') in caplog.record_tuples
    ...
    def test_warning_missing_label(self, setup, caplog):
        ...
        ModifyCommand().execute(args)
        assert ('cobib.commands.modify', 30, "No entry with the label 'dummy' could be found.") \
    ...
    def test_tui(self, setup, select, keys):
        ...
        def assertion(screen, logs, **kwargs):
            ...
            if kwargs.get('selection', False):
                expected_log.insert(2, ('cobib.commands.modify', 20, 'Selection given. '
                                        'Interpreting `filter` as a list of labels'))
            else:
                expected_log.insert(2, ('cobib.commands.modify', 10, 'Gathering filtered list of '
