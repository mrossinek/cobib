...
class TestDeleteCommand(CommandTest, TUITest):
    ...
    @pytest.mark.parametrize(['labels', 'skip_commit'], [
        [['knuthwebsite'], False],
        [['knuthwebsite', 'latexcompanion'], False],
        [['dummy'], True],  # non-existent labels should not cause any problems (but skip git check)
    ...
    @pytest.mark.parametrize(['select', 'keys', 'labels'], [
    ...
    def test_tui(self, setup, select, keys, labels):
        ...
        def assertion(screen, logs, **kwargs):
            ...
            # set)
            for label in labels:
                assert ('cobib.commands.delete', 10, f"Attempting to delete entry '{label}'.") \
                    in true_log
                assert ('cobib.commands.delete', 20, f"'{label}' was removed from the database.") \
                    in true_log

                # also assert that the label is no longer visible on screen
                assert all(label not in line for line in screen.display[1:-3])

            assert true_log[-1] == ('cobib.commands.delete', 10,
