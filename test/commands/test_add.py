...
class TestAddCommand(CommandTest, TUITest):
    ...
    @pytest.mark.parametrize(['more_args', 'entry_kwargs'], [
        [[], {}],
        [['-f', 'test/debug.py'], {'file': path.abspath('test/debug.py')}],
    ...
    def test_command(self, setup, more_args, entry_kwargs):
        ...
        args = ['-b', './test/commands/example_multi_file_entry.bib'] + more_args
        ...
        if entry_kwargs:
            ...
            self._assert('./test/commands/example_multi_file_entry.yaml')
    ...
    def test_add_new_entry(self, setup, caplog):
        ...
        assert ('cobib.commands.add', 30,
                "No input to parse. Creating new entry 'dummy' manually.") in caplog.record_tuples
    ...
    def test_overwrite_label(self, setup):
        ...
        AddCommand().execute(['-b', './test/commands/example_duplicate_entry.bib',
                              '--label', 'duplicate_resolver'])

        assert Database()['duplicate_resolver']

        self._assert('./test/commands/example_duplicate_entry.yaml')
    ...
    def test_cmdline(self, setup, monkeypatch):
        ...
        self.run_module(monkeypatch, 'main', ['cobib', 'add', '-b',
                                              './test/commands/example_multi_file_entry.bib'])
        self._assert('./test/commands/example_multi_file_entry.yaml')
    ...
    def test_tui(self, setup):
        ...
        def assertion(screen, logs, **kwargs):
            ...
            expected_log = [
                ...
                ('cobib.commands.add', 10,
                 "Adding entries from bibtex: './test/commands/example_multi_file_entry.bib'."),
        ...
        keys = 'a-b ./test/commands/example_multi_file_entry.bib\n'
