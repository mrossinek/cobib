...
class TUITest:
    ...
    def patch_curses(self, monkeypatch):
        ...
        monkeypatch.setattr('curses.curs_set', lambda *args:
                            self.LOGGER.debug('curs_set: %s', args))
        ...
        monkeypatch.setattr('curses.use_default_colors', lambda:
                            self.LOGGER.debug('use_default_colors'))
        monkeypatch.setattr('curses.start_color', lambda: self.LOGGER.debug('start_color'))
        monkeypatch.setattr('curses.init_pair', lambda *args:
                            self.LOGGER.debug('init_pair: %s', args))
        monkeypatch.setattr('curses.init_color', lambda *args:
                            self.LOGGER.debug('init_color: %s', args))
    ...
    def run_tui(keys, assertion, assertion_kwargs):
        ...
        if pid == 0:
            ...
            # redirect logging
            log_to_file(logging.DEBUG, '/tmp/cobib_tui_test.log')
            ...
            with open('/tmp/cobib_tui_test.log', 'r') as logfile:
                for line in logfile.readlines():
                    ...
                    if not line.startswith(str(date.today())):
                        ...
                        continue
                    line = line.split()
                    logs.append((line[3],  # source
                                 getattr(logging, line[2].strip('[]')),  # log level
                                 ' '.join(line[5:])  # message
                                 ))
            os.remove('/tmp/cobib_tui_test.log')
