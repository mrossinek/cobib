...
from cobib.tui import Frame, TextBuffer, TUI
from cobib.tui.state import Mode, STATE
...
class TestTUI(CmdLineTest, TUITest):
    ...
    @pytest.mark.parametrize(['can_change_color'], [
        [False],
        [True],
    ])
    def test_config_rgb_color(self, patch_curses, caplog, monkeypatch, can_change_color):
        ...
        if not can_change_color:
            assert ('cobib.tui.tui', 30, 'Curses cannot change the default colors. ' +
                    'Skipping color setup.') in caplog.record_tuples
        else:
    ...
    def test_config_unknown_color(self, patch_curses, caplog):
        ...
        TUI.colors()
        assert ('cobib.tui.tui', 30, 'Detected unknown TUI color name specification: dummy') \
    ...
    def test_config_bind_key(self, caplog):
        ...
        assert ('cobib.tui.tui', 20, 'Binding key p to the Prompt command.') \
            in caplog.record_tuples
    ...
    def test_config_unknown_command(self, caplog):
        ...
        TUI.bind_keys()
        assert ('cobib.tui.tui', 30, 'Unknown command "Dummy". Ignoring key binding.') \
            in caplog.record_tuples

    def test_infoline(self):
        """Test infoline method."""
        infoline = TUI.infoline()
        assert 'a:Add d:Delete e:Edit x:Export f:Filter ?:Help m:Modify o:Open ::Prompt q:Quit ' + \
            'r:Redo /:Search v:Select ENTER:Show s:Sort u:Undo w:Wrap' == infoline
    ...
    def test_init(self, patch_curses, caplog):
        ...
        assert 'a:Add d:Delete e:Edit x:Export f:Filter ?:Help m:Modify o:Open ::Prompt q:Quit ' + \
            'r:Redo /:Search v:Select ENTER:Show s:Sort u:Undo w:Wrap' == tui.botbar.lines[0]
        ...
        assert [record for record in caplog.record_tuples
                if record[0] in ('cobib.tui.tui', 'TUITest')] == expected_log
    ...
    def test_resize(self, patch_curses, caplog):
        ...
        expected_log = [
            ...
            ('MockCursesPad', 10, 'erase'),
            ('MockCursesPad', 10, 'getmaxyx'),
            ('MockCursesPad', 10, 'addnstr: 0 0 a:Add d:Delete e:Edit x:Export f:Filter ?:Help ' +
             'm:Modify o:Open ::Prompt q:Quit r:Redo /:Search v:Select ENTER:Show s:Sort u:Undo ' +
             'w:Wrap 69 0'),
            ('MockCursesPad', 10, 'refresh: None None None None None None'),
            ('MockCursesPad', 10, 'refresh: None None None None None None'),
            ('MockCursesPad', 10, 'resize: 1 70'),
        ...
        for log, truth in zip(expected_log, [record for record in caplog.record_tuples
    ...
    def test_help(self, patch_curses, caplog):
        ...
        for idx, (cmd, desc) in enumerate(TUI.HELP_DICT.items()):
            expected_log.insert(-6, ('MockCursesPad', 10, f"addstr: {3+idx} 1 " +
                                     "{:^8} {:<8} {}".format(
    ...
    def test_select(self, patch_curses, caplog, selection):
        ...
        if selection:
            ...
            expected_log.append(
                ('cobib.tui.tui', 20, "Adding 'knuthwebsite' to the selection.")
            )
        assert [record for record in caplog.record_tuples
                if record[0] in ('cobib.tui.frame', 'cobib.tui.tui')] == expected_log

    @pytest.mark.parametrize(['keys', 'assertion_kwargs'], [
            ...
            if not selected and not labels:
                for idx in range(1, 4):
                    assert [c.bg for c in screen.buffer[idx].values()] == \
                            ['cyan' if idx == current else 'default'] * term_width
            ...
            for idx, (line, truth) in enumerate(zip(screen.display[1:6], expected_screen)):
                if idx == 0:
                    assert [c.bg for c in screen.buffer[1].values()] == \
                        ['cyan'] * 6 + ['magenta'] * label_len + \
                        ['cyan'] * (term_width - label_len - 6)
    ...
    def test_select_search_live(self):
        ...
        def assertion(screen, logs, **kwargs):
            ...
            for idx, (line, truth) in enumerate(zip(screen.display[1:6], expected_screen)):
                if idx == 0:
                    assert [c.bg for c in screen.buffer[1].values()] == \
                        ['magenta'] * label_len + ['cyan'] * (term_width - label_len)
                assert line.strip() in truth.strip()
    ...
    @pytest.mark.parametrize(['prompt_quit', 'returned_char', 'mode'], [
    ...
    def test_quit(self, patch_curses, caplog, prompt_quit, returned_char, mode):
        ...
        if mode == Mode.LIST.value:
            ...
            expected_log.append(('cobib.tui.tui', 10,
    ...
    def test_loop_inactive_commands(self, patch_curses, caplog):
        ...
        tui.loop()
        # minimal assertions
        assert ('cobib.commands.show', 10, 'Show command triggered from TUI.') \
    ...
    def test_scroll_live(self, keys, assertion_kwargs):
        ...
        # overwrite database
        config.database.file = './test/tui/scrolling_database.yaml'
        try:
            Database().read()
            self.run_tui(keys, assertion, assertion_kwargs)
        finally:
