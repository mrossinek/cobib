...
@pytest.mark.parametrize(['_filter', '_or'], [
...
@pytest.mark.parametrize(['query', 'context', 'ignore_case', 'expected'], [
    ['search_query', 1, False, [
    ...
    ['search_query', 2, False, [
        ['@article{search_dummy,', ' abstract = {search_query', 'something else', 'Search_Query'],
        ['Search_Query', 'something else', 'search_query', 'something else', 'Search_Query'],
    ]],
    # the following will look almost identical to the second scenarios because otherwise the next
    # match would be included within the context.
    ['search_query', 2, True, [
        ['@article{search_dummy,', ' abstract = {search_query', 'something else'],
        ['something else', 'Search_Query', 'something else'],
        ['something else', 'search_query', 'something else'],
    ...
    # what we care about here, is that the second match does not include lines which occur *before*
    # the first match.
    ['search_query', 10, False, [
        ['@article{search_dummy,', ' abstract = {search_query', 'something else', 'Search_Query',
         'something else'],
        ['something else', 'Search_Query', 'something else', 'search_query', 'something else',
         'Search_Query', 'something else}', '}', '', ''],
    ]],
])
def test_search(query, context, ignore_case, expected):
    """Test search method."""
    entry = Entry('search_dummy', {
        'ENTRYTYPE': 'article',
        'ID': 'search_dummy',
        'abstract': '\n'.join([
            'search_query',
...
def test_escape_special_chars():
    ...
    entries = BibtexParser().parse('test/database/example_entry_umlaut.bib')
