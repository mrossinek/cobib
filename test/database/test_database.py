...
def test_database_missing_file(caplog):
    ...
    config.database.file = '/tmp/cobib_test_missing_file.yaml'
    try:
        with pytest.raises(SystemExit):
            Database().read()
        for (source, level, message) in caplog.record_tuples:
            if ('cobib.database.database', logging.CRITICAL) == (source, level) and \
                    f'The database file {config.database.file} does not exist!' in message:
                break
        else:
...
def test_database_save_add():
    ...
    config.database.file = '/tmp/cobib_test_database_file.yaml'
    copyfile('./test/example_literature.yaml', '/tmp/cobib_test_database_file.yaml')
    ...
    expected = []
    with open('./test/example_literature.yaml', 'r') as file:
    ...
    try:
        ...
        with open('/tmp/cobib_test_database_file.yaml', 'r') as file:
        ...
        os.remove('/tmp/cobib_test_database_file.yaml')
        config.database.file = './test/example_literature.yaml'
