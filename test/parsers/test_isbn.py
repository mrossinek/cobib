...
class TestISBNParser(ParserTest):
    ...
    def test_from_isbn(self, caplog):
        ...
        if any(s == 'cobib.parsers.isbn' and t == logging.ERROR
               for s, t, _ in caplog.record_tuples):
    ...
    def test_from_empty_isbn(self, caplog):
        ...
        assert ('cobib.parsers.isbn', logging.WARNING) \
            in [(source, level) for source, level, _ in caplog.record_tuples]
    ...
    def test_catching_api_error(self, caplog, monkeypatch):
        ...
        assert ('cobib.parsers.isbn', logging.ERROR,
    ...
    def test_catching_decode_error(self, caplog, monkeypatch):
        ...
        for record in caplog.record_tuples:
            if record[0] == 'cobib.parsers.isbn' and record[1] == logging.ERROR and \
                    'An Exception occurred while parsing the query results:' in record[2]:
                break
        else:
    ...
    def test_dump(self, caplog):
        ...
        assert ('cobib.parsers.isbn', logging.ERROR, 'Cannot dump an entry as an ISBN.') \
            in caplog.record_tuples
